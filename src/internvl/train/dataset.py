...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        PRM_TOKEN, SIGLIP_MEAN, SIGLIP_STD)
...
def preprocess_internlm(
    ...
    for i, source in enumerate(sources):
        ...
        conv.messages = []
        assert len(source) == 2, 'Must be a conversation with 2 turns'
        conv.append_message(roles[source[0]['from']], '')
        conv.append_message(roles[source[1]['from']], source[0]['value'].strip())
        conversations.append(conv.get_prompt())
        labels.append(source[1]['value'])
    ...
    targets.fill_(IGNORE_TOKEN_ID)
    targets[input_ids == PRM_TOKEN_ID] = labels
...
def preprocess_internvl2_5(
    ...
    labels = torch.tensor(conversations[1]['value'], dtype=torch.float)
