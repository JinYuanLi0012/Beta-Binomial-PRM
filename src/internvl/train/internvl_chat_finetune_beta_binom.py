...
# Beta-Binomial PRM
# =================
# This file implements beta-binomial PRM training with an explicit uncertainty head.
#
# Core design:
# - PRM positions are the '<prm>' placeholder tokens in the prompt.
# - Reward tokens are ['Yes', 'No'] from the tokenizer vocabulary.
# - Parameterization at each '<prm>' position:
#   - mu    = softmax([logit('Yes'), logit('No')])[0]
#   - kappa = softplus(kappa_head(h_prm)) + beta_binom_kappa_min
#            where h_prm is the '<prm>' token hidden state from the LLM.
#   - alpha = mu * kappa + beta_binom_eps
#   - beta  = (1 - mu) * kappa + beta_binom_eps
# - Training loss when prm_counts_k/n are provided:
#   - L_bb   = mean negative log Beta-Binomial pmf for (K,N | alpha,beta)
#   - L_evi  = beta_binom_evi_reg * mean(|mu.detach() - (K/N)| * kappa)
#   - L      = L_bb + L_evi
# - Fallback when counts are unavailable/invalid:
#   - CrossEntropyLoss over ['Yes','No'] logits with soft labels built from ratio target.
#
...
class BetaBinomStatsCallback(TrainerCallback):
    """Attach beta-binom batch statistics to Trainer logs/TensorBoard."""
...
def attach_kappa_head_grad_multiplier(model, mult: float):
    """Scale kappa_head gradients to emulate a higher LR without touching optimizer param groups."""
    if mult is None or float(mult) == 1.0:
        return
    if not hasattr(model, 'kappa_head'):
        return
    if hasattr(model, '_kappa_head_grad_hooks'):
        return
    handles = []
    scale = float(mult)
    for p in model.kappa_head.parameters():
        if not p.requires_grad:
            continue
        handles.append(p.register_hook(lambda g, s=scale: g * s))
...
class DataTrainingArguments:
    ...
    eval_meta_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Meta file path for evaluation dataset. If provided, enables eval.'},
    )

    max_eval_samples: Optional[int] = field(
        default=None,
        metadata={'help': 'Limit evaluation to first N samples per evaluate() call.'},
    )
    beta_binom_eps: float = field(
        default=1e-6,
        metadata={'help': 'Numerical epsilon used by beta-binomial loss.'},
    )
    # Kappa is predicted from a dedicated head. Keep a tiny lower bound for stability.
    beta_binom_kappa_min: float = field(
        default=1e-3,
        metadata={'help': 'Minimum kappa added after softplus (stability).'},
    )
    beta_binom_kappa_init: float = field(
        default=8.0,
        metadata={'help': 'Initial kappa value used to initialize kappa_head bias.'},
    )
    beta_binom_evi_reg: float = field(
        default=1e-2,
        metadata={'help': 'Evidence regularization strength for kappa (uses mu.detach()).'},
    )
    beta_binom_kappa_head_lr_mult: float = field(
        default=10.0,
        metadata={'help': 'Effective LR multiplier for kappa_head via gradient scaling.'},
    )

    # Backward-compatible knobs for older checkpoints.
    beta_binom_kappa_floor: float = field(
        default=2.0,
        metadata={'help': 'Minimum concentration floor added after softplus.'},
    )
    beta_binom_kappa_reg: float = field(
        default=1e-4,
        metadata={'help': 'L2 penalty strength on log(kappa).'},
    )
...
def concat_pad_data_collator_beta_binom(features, max_item_length=None, pad_id=0):
    """Pad prm_counts_k/n together with input_ids/labels for variable-length batches."""
    first = features[0]
    batch = {}
...
class LazySupervisedDataset(Dataset):
    ...
    def get_preprocess_function(self):
        ...
        if self.template_name == 'Hermes-2':
            ...
            preprocess_function = preprocess_internvl2_5_beta_binom
        else:
...
def main():
    ...
    model.beta_binom_kappa_reg = data_args.beta_binom_kappa_reg
    ...
    if data_args.eval_meta_path is not None:
        ...
        eval_dataset = build_datasets(
            ...
            max_dynamic_patch=eval_data_args.max_dynamic_patch,
            normalize_type=eval_data_args.normalize_type,
            min_num_frame=eval_data_args.min_num_frame,
            max_num_frame=eval_data_args.max_num_frame,
        )
    
    if eval_dataset is not None and data_args.max_eval_samples is not None:
        n = min(int(data_args.max_eval_samples), len(eval_dataset))
        eval_dataset = Subset(eval_dataset, list(range(n)))
    ...
    if data_args.use_packed_ds:
        collator = partial(
            packed_collate_fn,
            data_collator=concat_pad_data_collator_beta_binom,
        ...
        collator = concat_pad_data_collator_beta_binom
